...
export class WorldPhenologySystem {
  resolve(season: SeasonState, surface: EnvironmentSurfaceState, temperature: number, humidity: number, dayFactor: number): EnvironmentVisualState {
    ...
    const snow = clamp(surface.snowDepth * 1.3 + season.snowBias * 0.15, 0, 1);
    return {
      ...
      leafDrop: clamp(winter * 0.42 + autumn * 0.38 + heatDryness * 0.12, 0, 0.86),
      ...
      frost,
      snow,
      wetness: clamp(surface.wetness + surface.dew * 0.25, 0, 1),
      haze: clamp(heatDryness * 0.34 + humidity * 0.18 + surface.dew * 0.08, 0, 1),
